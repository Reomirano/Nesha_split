...
# KLJUČ ZA RESET (da polja budu prazna na klik)
if "reset_kljuc" not in st.session_state:
    st.session_state.reset_kljuc = 0

# --- SIDEBAR: LIČNA PODEŠAVANJA ---
...
# Inicijalizacija liste članova u memoriji pretraživača
...
with col_desno:
    ...
    st.number_input("Ukupan iznos (RSD):", value=suma_ukupno, disabled=True, key=f"ukupno_prikaz_{sufiks}")
        ...
        if not moje_ime or not moj_racun:
            ...
            for i, (ime, dug) in enumerate(finalni_dugovi.items()):
                if dug > 0:
                    ...
                    # QR podaci sada koriste varijable moje_ime i c_racun iz sidebar-a
